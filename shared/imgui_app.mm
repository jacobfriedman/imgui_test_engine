...
// [SECTION] ImGuiApp Implementation: GLFW + Metal
...
// Data
struct ImGuiApp_ImplGlfwMetal : public ImGuiApp
{
    GLFWwindow* window;
    CAMetalLayer *layer;
    id <MTLDevice> device;
...
static bool ImGuiApp_ImplGlfwMetal_CreateWindow(ImGuiApp* app_opaque, const char* window_title, ImVec2 window_size)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;
...
static void ImGuiApp_ImplGlfwMetal_InitBackends(ImGuiApp* app_opaque)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;

    app->device = MTLCreateSystemDefaultDevice();
    app->commandQueue = [app->device newCommandQueue];
    ...
    NSWindow *nswin = glfwGetCocoaWindow(app->window);
    app->layer = [CAMetalLayer layer];
    app->layer.device = app->device;
    app->layer.pixelFormat = MTLPixelFormatBGRA8Unorm;
    nswin.contentView.layer = app->layer;
    nswin.contentView.wantsLayer = YES;
    app->renderPassDescriptor = [MTLRenderPassDescriptor new];
}

static bool ImGuiApp_ImplGlfwMetal_NewFrame(ImGuiApp* app_opaque)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;
    ...
    app->layer.drawableSize = CGSizeMake(width, height);
    app->drawable = [app->layer nextDrawable];

    app->commandBuffer = [app->commandQueue commandBuffer];
    app->renderPassDescriptor.colorAttachments[0].clearColor = MTLClearColorMake(app->clear_color[0] * app->clear_color[3], app->clear_color[1] * app->clear_color[3], app->clear_color[2] * app->clear_color[3], app->clear_color[3]);
    app->renderPassDescriptor.colorAttachments[0].texture = app->drawable.texture;
    app->renderPassDescriptor.colorAttachments[0].loadAction = MTLLoadActionClear;
    app->renderPassDescriptor.colorAttachments[0].storeAction = MTLStoreActionStore;

    app->renderEncoder = [app->commandBuffer renderCommandEncoderWithDescriptor:app->renderPassDescriptor];
    [app->renderEncoder pushDebugGroup:@"ImGui App"];

    // Start the Dear ImGui frame
    ImGui_ImplMetal_NewFrame(app->renderPassDescriptor);
    ImGui_ImplGlfw_NewFrame();
    return true;
}

static void ImGuiApp_ImplGlfwMetal_Render(ImGuiApp* app_opaque)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;
    ImGuiIO& io = ImGui::GetIO();
    glfwMakeContextCurrent(app->window);
    ImGui_ImplMetal_RenderDrawData(ImGui::GetDrawData(), app->commandBuffer, app->renderEncoder);

    [app->renderEncoder popDebugGroup];
    [app->renderEncoder endEncoding];

    [app->commandBuffer presentDrawable:app->drawable];
    [app->commandBuffer commit];
...
static void ImGuiApp_ImplGlfwMetal_ShutdownCloseWindow(ImGuiApp* app_opaque)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;
    glfwDestroyWindow(app->window);
    glfwTerminate();
...
static void ImGuiApp_ImplGlfwMetal_ShutdownBackends(ImGuiApp* app_opaque)
{
    ImGuiApp_ImplGlfwMetal* app = (ImGuiApp_ImplGlfwMetal*)app_opaque;
    IM_UNUSED(app);
...
{
    ImGuiApp_ImplGlfwMetal* intf = new ImGuiApp_ImplGlfwMetal();
    intf->InitCreateWindow      = ImGuiApp_ImplGlfwMetal_CreateWindow;
    intf->InitBackends          = ImGuiApp_ImplGlfwMetal_InitBackends;
    intf->NewFrame              = ImGuiApp_ImplGlfwMetal_NewFrame;
    intf->Render                = ImGuiApp_ImplGlfwMetal_Render;
    intf->ShutdownCloseWindow   = ImGuiApp_ImplGlfwMetal_ShutdownCloseWindow;
    intf->ShutdownBackends      = ImGuiApp_ImplGlfwMetal_ShutdownBackends;
    intf->CaptureFramebuffer    = ImGuiApp_ImplNull_CaptureFramebuffer;
    intf->Destroy               = [](ImGuiApp* app) { delete (ImGuiApp_ImplGlfwMetal*)app; };
    return intf;
}

#endif // #ifdef IMGUI_APP_GLFW_METAL
...
static void ImGuiApp_InstalMockViewportsBackend(ImGuiApp*)
{
    ...
    memset((void*)&platform_io, 0, IM_OFFSETOF(ImGuiPlatformIO, Monitors)); // FIXME: Clear all handlers by default
